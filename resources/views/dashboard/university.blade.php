        ...
        <div class="col-md-12 col-lg-6">
            ...
            <div class="icon-cards-row mt-0">
                <div class="row mb-4">
                    ...
                    <div class="col-md-12 mb-4" style="margin-top: -26px;">
    ...
    <script>
                ...
                eventContent: function(arg) {
                    if (arg.event.classNames.includes('merged-event')) {
                        ...
                        const makeIconEl = (iconClasses, count, color, type) => {
                            ...
                            wrapper.style.color = '#c0bcbcff';

                            const icon = document.createElement('i');
                            icon.className = iconClasses; // e.g. 'fa-solid fa-bullhorn' or 'fas fa-bullhorn'
                        ...
                        const items = [
                            ...
                            makeIconEl('fa-solid fa-birthday-cake', birthdays.length, '#e966a5ff', 'birthday'),
                            makeIconEl('fa-solid fa-umbrella-beach', holidays.length, '#e97d4fff', 'holiday'),
                            makeIconEl('fa-solid fa-chalkboard-teacher', ptms.length, '#b48cf2ff', 'ptm')
                        ];
        ...
        `;
                                ...
                                if (type === 'ptm' && ptms.length > 0) {
                                    let html = ptms.map(p => `
                <div class="mb-3 border-bottom pb-2">
                    <div><strong>Title:</strong> ${p.title || 'Parent Teacher Meeting'}</div>
                    <div><strong>Date:</strong> ${new Date(p.ptmdate).toLocaleDateString()}</div>

                    <div><strong>Objective:</strong> ${p.objective || 'N/A'}</div>
                </div>
            `).join('');

                                    document.getElementById('announcementModalBody')
                                        .innerHTML = html;
                                    $('#announcementModalLabel').html('PTM');
                                    new bootstrap.Modal(document.getElementById(
                                        'announcementModal')).show();
                                }
