...
.assessment-summary, .milestone-summary, .eylf-summary {
    margin-bottom: 30px;
}
...
.subject-group, .outcome-group, .age-group {
    background: white;
    border-radius: 8px;
    padding: 20px;
    border: 1px solid #dee2e6;
...
.subject-title, .outcome-title, .age-group-title, .category-title {
    font-weight: 600;
    color: #495057;
    margin-bottom: 15px;
    padding-bottom: 10px;
...
.assessment-item, .outcome-item, .milestone-item {
    display: flex;
...
.assessment-item:last-child, .outcome-item:last-child, .milestone-item:last-child {
    border-bottom: none;
}
                                    ...
                                    <div class="info-card">
                                        ...
                                        <table class="table table-borderless">
                                            <tr>
                                                ...
                                                <td>{{ $observation->created_at->format('M d, Y') }}</td>
                                            </tr>
                                            <tr>
                                                <td><strong>Time:</strong></td>
                                                <td>{{ $observation->created_at->format('h:i A') }}</td>
                                            </tr>
                                            <tr>
                                                <td><strong>Status:</strong></td>
                                                <td>
                                                    <span class="badge badge-success">{{ ucfirst($observation->status ?? 'Active') }}</span>
                                                </td>
                                            </tr>
                                        </table>
                                    </div>
                                            ...
                                            <div class="media-gallery">
                                                @foreach($observation->media as $media)
                                                    <div class="media-item">
                                                        @if(in_array(pathinfo($media->mediaUrl, PATHINFO_EXTENSION), ['jpg', 'jpeg', 'png', 'gif']))
                                                            <img src="{{ asset($media->mediaUrl) }}" alt="Media" class="img-thumbnail">
                                                        @else
                                                            <div class="file-icon">
                                                ...
                                                <div class="col-md-6 mb-3">
                                                    <div class="child-card">
                                                        ...
                                                        <div class="child-info">
                                                            ...
                                                            <p class="text-muted mb-1">Age: {{ $childLink->child->age ?? 'N/A' }}</p>
                                        ...
                                        <div class="row">
                                            <div class="col-md-3">
                                                <div class="stat-card bg-danger">
                                                    <div class="stat-number">{{ $observation->montessoriLinks->where('assesment', 'Not Assessed')->count() }}</div>
                                                    <div class="stat-label">Not Assessed</div>
                                                </div>
                                            </div>
                                            <div class="col-md-3">
                                                <div class="stat-card bg-info">
                                                    <div class="stat-number">{{ $observation->montessoriLinks->where('assesment', 'Introduced')->count() }}</div>
                                                    <div class="stat-label">Introduced</div>
                                                </div>
                                            </div>
                                            <div class="col-md-3">
                                                <div class="stat-card bg-warning">
                                                    <div class="stat-number">{{ $observation->montessoriLinks->where('assesment', 'Working')->count() }}</div>
                                                    <div class="stat-label">Working</div>
                                                </div>
                                            </div>
                                            <div class="col-md-3">
                                                <div class="stat-card bg-success">
                                                    <div class="stat-number">{{ $observation->montessoriLinks->where('assesment', 'Completed')->count() }}</div>
                                                    ...
                                                    @foreach($assessments as $assessment)
                                                        <div class="assessment-item">
                                                            <div class="item-content">
                                                                ...
                                                                <div class="sub-item">{{ $assessment->subActivity->title ?? 'N/A' }}</div>
                                                            </div>
                                ...
                                @if($observation->eylfLinks && $observation->eylfLinks->count() > 0)
                                    <div class="eylf-summary mb-4">
                                        <div class="alert alert-info">
                                            ...
                                            Total EYLF outcomes achieved: <strong>{{ $observation->eylfLinks->count() }}</strong>
                                        </div>
                                    </div>
                                        ...
                                        @foreach($groupedByOutcome as $outcomeTitle => $links)
                                            <div class="outcome-group mb-4">
                                                <h6 class="outcome-title">
                                                    ...
                                                    <span class="badge badge-primary ml-2">{{ $links->count() }} achievements</span>
                                                </h6>
                                ...
                                @if($observation->devMilestoneSubs && $observation->devMilestoneSubs->count() > 0)
                                    <div class="milestone-summary mb-4">
                                        <div class="row">
                                            <div class="col-md-4">
                                                <div class="stat-card bg-info">
                                                    <div class="stat-number">{{ $observation->devMilestoneSubs->where('assessment', 'Introduced')->count() }}</div>
                                                    <div class="stat-label">Introduced</div>
                                                </div>
                                            </div>
                                            <div class="col-md-4">
                                                <div class="stat-card bg-warning">
                                                    <div class="stat-number">{{ $observation->devMilestoneSubs->where('assessment', 'Working towards')->count() }}</div>
                                                    <div class="stat-label">Working Towards</div>
                                                </div>
                                            </div>
                                            <div class="col-md-4">
                                                <div class="stat-card bg-success">
                                                    <div class="stat-number">{{ $observation->devMilestoneSubs->where('assessment', 'Achieved')->count() }}</div>
                                        ...
                                        @foreach($groupedByAgeGroup as $ageGroup => $milestones)
                                            <div class="age-group mb-4">
                                                <h6 class="age-group-title">
                                                    ...
                                                    <span class="badge badge-secondary ml-2">{{ $milestones->count() }} milestones</span>
                                                </h6>
                                                        ...
                                                        <div class="milestone-items">
                                                            @foreach($categoryMilestones as $milestone)
                                                                <div class="milestone-item">
                                                                    <div class="item-content">
                                                                        <div class="milestone-name">{{ $milestone->devMilestone->name ?? 'N/A' }}</div>
                                                                    </div>
                                                                    <div class="item-status">
                                                                        @php
                                                                            $statusClass = [
                                                                        ...
                                                                        @endphp
                                                                        <span class="badge {{ $statusClass[$milestone->assessment] ?? 'badge-secondary' }}">
