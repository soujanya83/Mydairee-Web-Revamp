...
<div class="col-lg-12 col-md-12">
    ...
    <li class="nav-item">
        <a class="nav-link active show" data-toggle="tab" href="#Home">
            <i class="icon-camera"></i> <span>Snapshots</span>
        </a>
    </li>
</ul>
...
function handleObservationStatusChange(status) {
    ...
    $.ajax({
        url: "{{ route('snapshot.status.update') }}",
        method: 'POST',
        data: {
            _token: "{{ csrf_token() }}",
            reflectionId: obsId,
        ...
        success: function(res) {
            if (res.status === 'success') {
                showToast('success', `snapshot marked as ${status}`);
                $button.html('Saved!');
                setTimeout(() => {
                    window.location.href = "{{ route('snapshot.index') }}";
                }, 1500);
            } else {
