...
<!-- resources/views/program_plan_list.blade.php -->
   ...
   <div class="main-container">
    ...
    @forelse($programPlans as $index => $plan)
        <tr>
            <td data-label="ID">
                <span class="id-badge">{{ ($programPlans->currentPage() - 1) * $programPlans->perPage() + $loop->iteration }}
</span>
            </td>
            <td data-label="Month">
            ...
            <td data-label="Created Date">
                <span class="date-text">{{ \Carbon\Carbon::parse($plan->created_at)->format('d M Y / H:i') }}</span>
            </td>
            <td data-label="Updated Date">
                <span class="date-text">{{ \Carbon\Carbon::parse($plan->updated_at)->format('d M Y / H:i') }}</span>
            </td>
            <td data-label="Actions">
                <div class="action-buttons">
                    <a href="{{ route('print.programplan', $plan->id) }}" class="btn btn-action btn-print" title="Print">
                        <i class="fas fa-print animated-icon"></i>
                    </a>

                    @if(Auth::user()->userType != 'Parent')
                        <a href="{{ route('create.programplan', ['centerId' => $centerId, 'planId' => $plan->id]) }}" class="btn btn-action btn-edit" title="Edit">
                            <i class="fas fa-pen-to-square animated-icon"></i>
                        </a>

                        <button type="button" class="btn btn-action btn-delete delete-program" data-id="{{ $plan->id }}" title="Delete">
