                        ...
                        @if ($userPermissions)
                            @php
                                function getPermissionIcon($label)
                                {
                                    ...
                                    if (str_contains($labelLower, 'add')) {
                                        return 'fas fa-plus-circle';
                                    }
                                    if (str_contains($labelLower, 'approve')) {
                                        return 'fas fa-check-circle';
                                    }
                                    if (str_contains($labelLower, 'delete')) {
                                        return 'fas fa-trash-alt';
                                    }
                                    if (str_contains($labelLower, 'update') || str_contains($labelLower, 'edit')) {
                                        return 'fas fa-edit';
                                    }
                                    if (str_contains($labelLower, 'view')) {
                                        return 'fas fa-eye';
                                    }
                            ...
                            <form action="{{ route('settings.update-permission') }}" method="post">
                                        ...
                                        <div class="permission-card">
                                            <div class="card-header">
                                                ...
                                                <div class="permission-count">
                                                    <i class="fas fa-check"></i>
                                                    {{ $ObservationPermissions->filter(fn($perm) => $userPermissions->{$perm['name']} ?? false)->count() }}
                                                    / {{ $ObservationPermissions->count() }}
                                                </div>
                                            </div>
                                            <div class="card-body">
                                    ...
                                    @if ($ReflectionPermissions->count() > 0)
                                        <div class="permission-card">
                                            <div class="card-header">
                                                ...
                                                <div class="permission-count">
                                                    <i class="fas fa-check"></i>
                                                    {{ $ReflectionPermissions->filter(fn($perm) => $userPermissions->{$perm['name']} ?? false)->count() }}
                                                    / {{ $ReflectionPermissions->count() }}
                                                </div>
                                            </div>
                                            <div class="card-body">
                                    ...
                                    @if ($QipPermissions->count() > 0)
                                        <div class="permission-card">
                                            <div class="card-header">
                                                ...
                                                <div class="permission-count">
                                                    <i class="fas fa-check"></i>
                                                    {{ $QipPermissions->filter(fn($perm) => $userPermissions->{$perm['name']} ?? false)->count() }}
                                                    / {{ $QipPermissions->count() }}
                                                </div>
                                            </div>
                                            <div class="card-body">
                                    ...
                                    @if ($RoomPermissions->count() > 0)
                                        <div class="permission-card">
                                            <div class="card-header">
                                                ...
                                                <div class="permission-count">
                                                    <i class="fas fa-check"></i>
                                                    {{ $RoomPermissions->filter(fn($perm) => $userPermissions->{$perm['name']} ?? false)->count() }}
                                                    / {{ $RoomPermissions->count() }}
                                                </div>
                                            </div>
                                            <div class="card-body">
                                    ...
                                    @if ($AnnouncementPermissions->count() > 0)
                                        <div class="permission-card">
                                            <div class="card-header">
                                                ...
                                                <div class="permission-count">
                                                    <i class="fas fa-check"></i>
                                                    {{ $AnnouncementPermissions->filter(fn($perm) => $userPermissions->{$perm['name']} ?? false)->count() }}
                                                    / {{ $AnnouncementPermissions->count() }}
                                                </div>
                                            </div>
                                            <div class="card-body">
                                    ...
                                    @if ($SurveyPermissions->count() > 0)
                                        <div class="permission-card">
                                            <div class="card-header">
                                                ...
                                                <div class="permission-count">
                                                    <i class="fas fa-check"></i>
                                                    {{ $SurveyPermissions->filter(fn($perm) => $userPermissions->{$perm['name']} ?? false)->count() }}
                                                    / {{ $SurveyPermissions->count() }}
                                                </div>
                                            </div>
                                            <div class="card-body">
                                    ...
                                    @if ($RecipePermissions->count() > 0)
                                        <div class="permission-card">
                                            <div class="card-header">
                                                ...
                                                <div class="permission-count">
                                                    <i class="fas fa-check"></i>
                                                    {{ $RecipePermissions->filter(fn($perm) => $userPermissions->{$perm['name']} ?? false)->count() }}
                                                    / {{ $RecipePermissions->count() }}
                                                </div>
                                            </div>
                                            <div class="card-body">
                                    ...
                                    @if ($MenuPermissions->count() > 0)
                                        <div class="permission-card">
                                            <div class="card-header">
                                                ...
                                                <div class="permission-count">
                                                    <i class="fas fa-check"></i>
                                                    {{ $MenuPermissions->filter(fn($perm) => $userPermissions->{$perm['name']} ?? false)->count() }}
                                    ...
                                    @if ($ProgramPlanPermissions->count() > 0)
                                        <div class="permission-card">
                                            <div class="card-header">
                                                ...
                                                <div class="permission-count">
                                                    <i class="fas fa-check"></i>
                                                    {{ $ProgramPlanPermissions->filter(fn($perm) => $userPermissions->{$perm['name']} ?? false)->count() }}
                                                    / {{ $ProgramPlanPermissions->count() }}
                                                </div>
                                            </div>
                                            <div class="card-body">
                                    ...
                                    @if ($DailyPermissions->count() > 0)
                                        <div class="permission-card">
                                            <div class="card-header">
                                                ...
                                                <div class="permission-count">
                                                    <i class="fas fa-check"></i>
                                                    {{ $DailyPermissions->filter(fn($perm) => $userPermissions->{$perm['name']} ?? false)->count() }}
                                                    / {{ $DailyPermissions->count() }}
                                                </div>
                                            </div>
                                            <div class="card-body">
                                    ...
                                    @if ($UsersPermissions->count() > 0)
                                        <div class="permission-card">
                                            <div class="card-header">
                                                ...
                                                <div class="permission-count">
                                                    <i class="fas fa-check"></i>
                                                    {{ $UsersPermissions->filter(fn($perm) => $userPermissions->{$perm['name']} ?? false)->count() }}
                                                    / {{ $UsersPermissions->count() }}
                                                </div>
                                            </div>
                                            <div class="card-body">
                                    ...
                                    @if ($CentersPermissions->count() > 0)
                                        <div class="permission-card">
                                            <div class="card-header">
                                                ...
                                                <div class="permission-count">
                                                    <i class="fas fa-check"></i>
                                                    {{ $CentersPermissions->filter(fn($perm) => $userPermissions->{$perm['name']} ?? false)->count() }}
                                                    / {{ $CentersPermissions->count() }}
                                                </div>
                                            </div>
                                            <div class="card-body">
                                    ...
                                    @if ($ChildPermissions->count() > 0)
                                        <div class="permission-card">
                                            <div class="card-header">
                                                ...
                                                <div class="permission-count">
                                                    <i class="fas fa-check"></i>
                                                    {{ $ChildPermissions->filter(fn($perm) => $userPermissions->{$perm['name']} ?? false)->count() }}
                                                    / {{ $ChildPermissions->count() }}
                                                </div>
                                            </div>
                                            <div class="card-body">
                                    ...
                                    @if ($ParentPlanPermissions->count() > 0)
                                        <div class="permission-card">
                                            <div class="card-header">
                                                ...
                                                <div class="permission-count">
                                                    <i class="fas fa-check"></i>
                                                    {{ $ParentPlanPermissions->filter(fn($perm) => $userPermissions->{$perm['name']} ?? false)->count() }}
                                                    / {{ $ParentPlanPermissions->count() }}
                                                </div>
                                            </div>
                                            <div class="card-body">
                                    ...
                                    @if ($ProgressPermissions->count() > 0)
                                        <div class="permission-card">
                                            <div class="card-header">
                                                ...
                                                <div class="permission-count">
                                                    <i class="fas fa-check"></i>
                                                    {{ $ProgressPermissions->filter(fn($perm) => $userPermissions->{$perm['name']} ?? false)->count() }}
                                                    / {{ $ProgressPermissions->count() }}
                                                </div>
                                            </div>
                                            <div class="card-body">
                                    ...
                                    @if ($LessonPermissions->count() > 0)
                                        <div class="permission-card">
                                            <div class="card-header">
                                                ...
                                                <div class="permission-count">
                                                    <i class="fas fa-check"></i>
                                                    {{ $LessonPermissions->filter(fn($perm) => $userPermissions->{$perm['name']} ?? false)->count() }}
                                                    / {{ $LessonPermissions->count() }}
                                                </div>
                                            </div>
                                            <div class="card-body">
                                    ...
                                    @if ($AssessmentPermissions->count() > 0)
                                        <div class="permission-card">
                                            <div class="card-header">
                                                ...
                                                <div class="permission-count">
                                                    <i class="fas fa-check"></i>
                                                    {{ $AssessmentPermissions->filter(fn($perm) => $userPermissions->{$perm['name']} ?? false)->count() }}
                                                    / {{ $AssessmentPermissions->count() }}
                                                </div>
                                            </div>
                                            <div class="card-body">
                                    ...
                                    @if ($AccidentsPermissions->count() > 0)
                                        <div class="permission-card">
                                            <div class="card-header">
                                                ...
                                                <div class="permission-count">
                                                    <i class="fas fa-check"></i>
                                                    {{ $AccidentsPermissions->filter(fn($perm) => $userPermissions->{$perm['name']} ?? false)->count() }}
                                                    / {{ $AccidentsPermissions->count() }}
                                                </div>
                                            </div>
                                            <div class="card-body">
                                    ...
                                    @if ($SnapshotsPermissions->count() > 0)
                                        <div class="permission-card">
                                            <div class="card-header">
                                                ...
                                                <div class="permission-count">
                                                    <i class="fas fa-check"></i>
                                                    {{ $SnapshotsPermissions->filter(fn($perm) => $userPermissions->{$perm['name']} ?? false)->count() }}
                                                    / {{ $SnapshotsPermissions->count() }}
                                                </div>
                                            </div>
                                            <div class="card-body">
                                    ...
                                    @if ($ActivitiesPermissions->count() > 0)
                                        <div class="permission-card">
                                            <div class="card-header">
                                                ...
                                                <div class="permission-count">
                                                    <i class="fas fa-check"></i>
                                                    {{ $ActivitiesPermissions->filter(fn($perm) => $userPermissions->{$perm['name']} ?? false)->count() }}
                                                    / {{ $ActivitiesPermissions->count() }}
                                                </div>
                                            </div>
                                            <div class="card-body">
                                    ...
                                    @if ($PTMPermissions->count() > 0)
                                        <div class="permission-card">
                                            <div class="card-header">
                                                ...
                                                <div class="permission-count">
                                                    <i class="fas fa-check"></i>
                                                    {{ $PTMPermissions->filter(fn($perm) => $userPermissions->{$perm['name']} ?? false)->count() }}
                                                    / {{ $PTMPermissions->count() }}
                                                </div>
                                            </div>
                                            <div class="card-body"> 
                                                @foreach ($PTMPermissions as $perm)
                                                    <div class="permission-item">
                                                        <label>
                                                            <i class="{{ getPermissionIcon($perm['label']) }}"></i>
                                                            {{ $perm['label'] }}
                                    ...
                                    @if ($otherPermissions->count() > 0)
                                        <div class="permission-card">
                                            <div class="card-header">
                                                ...
                                                <div class="permission-count">
                                                    <i class="fas fa-check"></i>
                                                    {{ $otherPermissions->filter(fn($perm) => $userPermissions->{$perm['name']} ?? false)->count() }}
                                                    / {{ $otherPermissions->count() }}
                                                </div>
                                            </div>
                                            <div class="card-body">
                                ...
                                <div class="submit-btn float-right" style="position:relative; z-index:9999; bottom:35px;">
        ...
        $(function() {
            ...
            $(document).on('change', '.permission-toggle', function() {
               // console.log('permission changed:', this.dataset.permission, 'checked=', this
                ...
                $card.find('.permission-count').html(
                    `<i class="fas fa-check"></i> ${checkedCount} / ${totalCount}`);
