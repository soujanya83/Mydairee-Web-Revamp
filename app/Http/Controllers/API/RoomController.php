...
class RoomController extends Controller
{
    ...
    public function filterChildren(Request $request)
    {
        ...
        $searchInput = trim((string) $request->input('search', ''));
        ...
        try {
            ...
            if ($searchInput !== '') {
                $query->where(function ($q) use ($searchInput) {
                    $q->where('name', 'like', '%' . $searchInput . '%')
                        ...
                        ->orWhereRaw("CONCAT(COALESCE(name, ''), ' ', COALESCE(lastname, '')) LIKE ?", ['%' . $searchInput . '%']);
                });
            }
            ...
            // Get filtered children with pagination
            ...
            return response()->json([
                'status' => true,
                'message' => $children->total() > 0 ? 'Children filtered successfully.' : 'No children found matching the filters.',
