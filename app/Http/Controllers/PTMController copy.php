...
class PTMController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            foreach ($selectedDate as $date) {
                //becuse sql not accept format so changed 
                $date = \Carbon\Carbon::createFromFormat('d-m-Y', trim($date))->format('Y-m-d');
            ...
            // $selectedSlot = $request->input('add_slot');
    ...
    public function edit(PTM $ptm)
    {
        ...
        //print_r($currentSlot = $ptm->slot);die();
    ...
    public function view(PTM $ptm)
    {
        ...
        $finalSlot = $latestReschedule && $latestReschedule->rescheduleslot
            ...
            : ($ptm->ptmSlots->first()->slot ?? null);

        // Original (before reschedule)
    ...
    {
         $centerId = session('user_center_id');
        // Return PTM events by using related ptmDates (the app stores dates in ptmdate table)
        $ptms = PTM::with('ptmDates')
            ->when($centerId, fn($q) => $q->where('centerid', $centerId))
            ->where('status', 'Published')
            ->get()
            ->map(function ($ptm) {
                $firstDate = $ptm->ptmDates->min('date');
                $ptm->ptmdate = $firstDate ? \Carbon\Carbon::parse($firstDate)->format('Y-m-d') : null;
                return [
                    'id' => $ptm->id,
                    'title' => $ptm->title,
                    'ptmdate' => $ptm->ptmdate,
    ...
    public function ptmDetails($id)
    {
        ...
        $childrenData = $ptm->children->map(function ($child) use ($ptm, $defaultDate, $defaultSlot) {
            ...
            // 🔹 Prepare full reschedule history for this child
                $history = $childReschedules->map(function ($res) {
                    return [
                        ...
                        'previous_date' => $res->previous_date ?? 'N/A',
                        'previous_slot' => $res->previous_slot ?? 'N/A',
                    ];
                })->values();
    ...
    {
        $ptm = PTM::with(['ptmDates', 'ptmSlots'])->findOrFail($ptmId);
        $child = Child::findOrFail($childId); // assuming you have a Child model

        // Get available dates and slots
        $availableDates = $ptm->ptmDates;
        $availableSlots = $ptm->ptmSlots;
        // dd($availableDates,$availableSlots);

        return view('ptm.staffres', compact('ptm', 'child', 'availableDates', 'availableSlots'));
    }
